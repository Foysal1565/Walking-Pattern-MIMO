...
class BeamLearn(object):
    ...
    def load_data(self):
        ...
        if not os.path.exists(self.args.save_path + '/indexes_beamlearn.pkl'):
            ...
            indexes = np.arange(
                self.num_samples_tot_gain_tx_beam * self.num_beams * self.num_gains
            )

            train_idxs, valid_idxs, test_idxs = [], [], []
            train, val = self.train_perc, self.train_perc + self.valid_perc
            num_train = int(self.num_samples_tot_gain_tx_beam * train)
            num_val = int(self.num_samples_tot_gain_tx_beam * val)
            ...
            for gain in gains:
                for beam in range(self.num_beams):
                    start_idx = gain * self.num_beams * self.num_samples_tot_gain_tx_beam + beam * self.num_samples_tot_gain_tx_beam
                    train_idxs.extend(
                    ...
                    test_idxs.extend(
                        indexes[start_idx+num_val: start_idx+self.num_samples_tot_gain_tx_beam]
                    )
